...
function offlineResult(prompt) {
  return `
OpenAI Realtime Language Tutor offline realtime session plan
...
export async function POST(request) {
  ...
  const prompt = typeof body.prompt === "string" ? body.prompt.trim() : "A voice language tutor starter with session setup, speaking goals, and live practice scaffolding.";
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/realtime/sessions", {
      ...
      body: JSON.stringify({
        ...
        instructions: `A voice language tutor starter with session setup, speaking goals, and live practice scaffolding.\n\nKeep responses concise, helpful, and safe.`,
      }),
    });
